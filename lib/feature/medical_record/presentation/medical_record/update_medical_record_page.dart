...
class UpdateMedicalRecordPage extends StatefulWidget {
  ...
  const UpdateMedicalRecordPage({super.key, required this.patientId});

  @override
  State<UpdateMedicalRecordPage> createState() =>
      _UpdateMedicalRecordPageState();
}
...
class _UpdateMedicalRecordPageState extends State<UpdateMedicalRecordPage> {
  ...
  Future<void> _loadRecord() async {
    ...
    await viewModel.getMedicalRecord(widget.patientId);
    if (!mounted) return;
    _populateFields(viewModel.state);
  }

  void _populateFields(MedicalRecordState state) {
    final record = state.medicalRecord;
    if (record == null || _fieldsInitialized) return;

    setState(() {
      _fieldsInitialized = true;
      _weightController.text = _formatWeight(record.weight);
      _heightController.text = record.height.toString();
      _consultationReasonController.text = record.consultationReason;
      _observationsController.text = record.observations;
      _patientHistories
        ..clear()
        ..addAll(record.patientHistories);
      _symptoms
        ..clear()
        ..addAll(record.symptoms);
    });
  }
  ...
  Future<void> _submit() async {
    final viewModel = context.read<MedicalRecordViewModel>();
    await viewModel.updateMedicalRecord(
    ...
    final error = viewModel.state.updateErrorMessage;
    if (error == null) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Edite los datos de la consulta para el seguimiento del paciente.',
                style: TextStyle(fontSize: 13, color: Colors.grey[600]),
              ),
              const SizedBox(height: 20),
              Expanded(
                child: !_fieldsInitialized && state.isLoadingRecord
                    ? const Center(
                        child: CircularProgressIndicator(
                          color: Color(0xFF0D6EA8),
                        ),
                      )
                    : !_fieldsInitialized && state.recordErrorMessage != null
                    ? Center(
                        child: Padding(
              ...
              if (_fieldsInitialized)
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: state.isUpdatingRecord || !_canSubmit
                        ? null
                        : _submit,
                    icon: state.isUpdatingRecord
                        ? const SizedBox(
                            width: 18,
                            height: 18,
                            child: CircularProgressIndicator(
