...
import 'package:ferova_clinic_flutter/feature/treatment/data/dtos/risk_patients_response_dto.dart';
...
class TreatmentService {
  ...
  Future<RiskPatientsResponseDto> getPatientsByRiskLevel(String riskLevel, String token) async {
    try {

      final Uri uri = Uri.parse('$_baseUrl/risk/$riskLevel/patients');
      final response = await http.get(uri, headers: _headers(token));

      if (response.statusCode == HttpStatus.ok) {
        final json = jsonDecode(response.body) as Map<String, dynamic>;
        return RiskPatientsResponseDto.fromJson(json);
      }
      throw Exception('Failed to fetch patients by risk level. ${response.body}');
